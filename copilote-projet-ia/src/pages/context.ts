...
const DEFAULT_CONTEXT: ProjectContext = {
  ...
  preferences: { language: 'Français', detailLevel: 'Standard', communicationStyle: 'Professionnel et direct' },
...
export function renderContextPage(root: Element, options: ContextPageOptions): void {
  ...
  // --- Section Projet ---
  const nameField = createTextField({ id: 'project-name', label: 'Nom du projet', value: existing.project.name, required: true });
  ...
  const sectorField = createTextField({ id: 'project-sector', label: 'Secteur', value: existing.project.businessSector });
  const teamField = createTextField({ id: 'project-team', label: 'Équipe', value: existing.project.team });
  const stakeholdersField = createTextField({
    id: 'project-stakeholders',
    label: 'Parties prenantes',
    value: existing.project.stakeholders,
  });
  const sponsorField = createTextField({ id: 'project-sponsor', label: 'Sponsor', value: existing.project.sponsor });
  ...
  // --- Section Préférences ---
  const languageField = createTextField({ id: 'language', label: 'Langue', value: existing.preferences.language });
  ...
  function collectContext(): ProjectContext {
    return {
      profile: { ...existing.profile, role: roleSelect.select.value as ProjectContext['profile']['role'] },
      project: {
  ...
  const form = el('form', { class: 'context-form' }, [
    ...
    el('fieldset', { class: 'field-group' }, [el('legend', {}, ['Méthodologie']), methodologySelect.field]),
    el('fieldset', { class: 'field-group' }, [
